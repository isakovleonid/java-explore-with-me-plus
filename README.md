...
public class CommentFullDto {
    ...
    State state; // Текущий статус комментария (APPROVED, REJECTED, PENDING)
}
```
...
Редактирует существующий комментарий пользователя. При изменении одобренного комментария его статус меняется на `PENDING`.
...
*   `status`: `String` - Статус комментариев для фильтрации (`"APPROVED"`, `"REJECTED"`, `"PENDING"`, `"ALL"`). **Обязательный.**
...
*   Если запрошен чужой комментарий, возвращается `CommentShortDto`, при этом комментарий должен быть опубликованным (`APPROVED`).
...
*   **`200 OK`** - Успешное получение комментария.
    *   **Тело ответа:**
        *   `CommentFullDto` (если запрашивается свой комментарий).
        *   `CommentShortDto` (если запрашивается чужой опубликованный комментарий).
*   **`403 Forbidden`** - Запрошен чужой неопубликованный (не `APPROVED`) комментарий.
*   **`404 Not Found`** - Пользователь, событие или комментарий не найдены.
...
Получает список всех опубликованных (`APPROVED`) комментариев к указанному событию.
...
**Ограничения:**
*   `status` должен быть одним из указанных значений.
